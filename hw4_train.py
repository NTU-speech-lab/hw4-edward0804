...
def train_word2vec(x):
    ...
    model = word2vec.Word2Vec(x, size=300, window=4, min_count=6, workers=multiprocessing.cpu_count(), iter=25, sg=1)
    return model
...
my_train_loss = []
my_val_loss = []
def training(batch_size, n_epoch, lr, model_dir, train, valid, model, device,pth_name):
    ...
    t_batch = len(train) 
    v_batch = len(valid) 
    optimizer = optim.Adam(model.parameters(), lr=lr,amsgrad=True) # 將模型的參數給 optimizer，並給予適當的 learning rate
    ...
    for epoch in range(n_epoch):
        ...
        my_epoch.append(epoch+1)
        my_trainAcc.append(total_acc/t_batch)
        my_train_loss.append(total_loss/t_batch)
        ...
        with torch.no_grad():
            ...
            my_valAcc.append(total_acc/v_batch)
            my_val_loss.append(total_loss/v_batch)
            if total_acc > best_acc:
                # 如果 validation 的結果優於之前所有的結果，就把當下的模型存下來以備之後做預測時使用
                best_acc = total_acc
                #torch.save(model, "{}/val_acc_{:.3f}.model".format(model_dir,total_acc/v_batch*100))
                torch.save(model, pth_name.format(model_dir))
...
def testing(batch_size, test_loader, model, model1, device,):
    model.eval()
    model1.eval()
    ret_output = []
    with torch.no_grad():
...
# 製作一個 model 的對象
model = LSTM_Net(embedding, embedding_dim=300, hidden_dim=650, num_layers=3, dropout=0.5, fix_embedding=fix_embedding)
...
# 開始訓練
training(batch_size, epoch_pretrain, lr, model_dir, train_loader, val_loader, model, device,pth_name)
# 把 data 分為 training data 跟 validation data（將一部份 training data 拿去當作 validation data）
X_train, X_val, y_train, y_val = train_x[-180000:], train_x[:-180000], y[-180000:], y[:-180000]
...
model = model.to(device) # device為 "cuda"，model 使用 GPU 來訓練（餵進去的 inputs 也需要是 cuda tensor）
# 開始訓練
training(batch_size, epoch_pretrain, lr, model_dir, train_loader, val_loader, model, device,pth_name1)